...
function isoToUtcYmd(iso) {
  ...
  // Fallback: if parsing fails, try to keep the original YYYY-MM-DD prefix.
  return iso.length >= 10 ? iso.slice(0, 10) : '';
}

function formatIsoForSticker(iso) {
  if (!iso || typeof iso !== 'string') return '—';
  // Keep it consistent with the backend: show UTC.
  // IMPORTANT: Parse ISO strings with Date so timezone offsets like "+08:00"
  // are converted correctly before we label the output as UTC.
  const d = new Date(iso);
  if (!Number.isNaN(d.getTime())) {
    const ymd = isoToUtcYmd(iso);
    const hh = String(d.getUTCHours()).padStart(2, '0');
    const mm = String(d.getUTCMinutes()).padStart(2, '0');
    return `${ymd} ${hh}:${mm} UTC`;
  }

  // Fallback: if Date parsing fails, keep the old string-based behavior.
...
function downloadAppointmentStickerPdf({
  ...
  const safeDate = isoToUtcYmd(scheduledForIso) || 'date';
